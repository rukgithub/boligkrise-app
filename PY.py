...
def main():
    # --- 1. Indlæs data ---
    # CSV-data (tab-separeret) med udviklingstal for kommuner
...
"""
    ...
    # --- 2. Hent og merge geodata ---
    # Hent GeoJSON-data for danske kommuner fra den nye URL
    ...
    # Merge geodata med CSV-data ved at matche 'label_dk' (GeoJSON) med 'Kommune' (CSV)
    merged = gdf.merge(df, left_on="label_dk", right_on="Kommune", how="left")
    
    # --- 3. Beregn kortgrænser for hele Danmark og for de 5 regioner ---
    overall_bounds = merged.total_bounds  # [minx, miny, maxx, maxy]
    # Omregn til format: [[south, west], [north, east]]
    region_bounds = {}
    region_bounds["Hele Danmark"] = [[overall_bounds[1], overall_bounds[0]], [overall_bounds[3], overall_bounds[2]]]
    
    # De 5 regioner (brug de officielle regioner fra CSV-kolonnen "Region")
    ...
    for reg in regions:
        ...
        if not sub.empty:
            b = sub.total_bounds  # [minx, miny, maxx, maxy]
            region_bounds[reg] = [[b[1], b[0]], [b[3], b[2]]]
    
    # --- 4. Opret kort med choropleth, tooltips, søgefunktion og regionsdropdown ---
    m = folium.Map(location=[56, 10], zoom_start=7)
    
    # Tilføj et choropleth-lag baseret på pct_change_2010_2024
    folium.Choropleth(
    ...
    # Tilføj et GeoJSON-lag med tooltips
    ...
    # Tilføj søgefunktion (søger i 'label_dk' på GeoJSON-laget)
    Search(
        layer=geojson_layer,
        geom_type="Polygon",
    ...
    # --- 5. Tilføj dropdown-menu til regionsvalg ---
    # Udarbejd HTML/JS-kode med en dropdown, der indeholder "Hele Danmark" og de 5 regioner
    dropdown_template = Template(u"""
    ...
    <div id="regionSelectContainer">
        <label for="regionSelect"><b>Vælg region:</b></label>
        <select id="regionSelect">
            {% for key in region_bounds.keys() %}
                <option value="{{ key }}">{{ key }}</option>
            {% endfor %}
        </select>
    </div>
    <script>
      // Få fat i kortets variabelnavn (brug det navn, folium genererer)
      var map_object = {{ this._parent.get_name() }};
      var regionBounds = {{ region_bounds|tojson }};
      document.getElementById('regionSelect').addEventListener('change', function(e) {
          var region = e.target.value;
          if(region in regionBounds) {
    ...
    """)
    macro = MacroElement()
    # Vi sender region_bounds til templaten
    macro._template = Template(dropdown_template.render(region_bounds=region_bounds))
    m.get_root().add_child(macro)
    
    folium.LayerControl().add_to(m)
    
    # --- 6. Gem kortet ---
    output_path = "C:/Users/ruk/OneDrive - Green Power Denmark/APP Stork/Kampagne/bolig/DST tal/Data til SoMe/danmarkskort_ui.html"
    m.save(output_path)
    print(f"Danmarkskort med UI gemt som '{output_path}'")
