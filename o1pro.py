...
Formål:
Lav et interaktivt Folium-kort over ALLE danske kommuner
baseret på 'label_dk' i geoJSON'en, farvelagt efter
pct_change_2010_2024 (ændringen i andelen af hjemmeboende
20-24-årige fra 2010 til 2024).

Resultat:
Gemmer filen 'danmarkskort.html' i:
  C:/Users/ruk/OneDrive - Green Power Denmark/APP Stork/Kampagne/bolig/DST tal/Data til SoMe
...
# -------------------------------------------------------------------------
# 1) Indlejret data (tab-separeret + komma som decimalseparator)
# -------------------------------------------------------------------------
...
# Læs data i en DataFrame
df = pd.read_csv(StringIO(data), sep="\t", decimal=",")

# -------------------------------------------------------------------------
# 2) Hent geodata fra den nye URL
# -------------------------------------------------------------------------
print("Henter GeoJSON data...")
...
print("GeoJSON kolonner:", gdf.columns.tolist())

# -------------------------------------------------------------------------
# 3) Merge data
#    - Kolonnen i geojson = "label_dk"
#    - Kolonnen i df      = "Kommune"
# -------------------------------------------------------------------------
print("Merger data...")
merged = gdf.merge(df, left_on="label_dk", right_on="Kommune", how="left")
print("Merged kolonner:", merged.columns.tolist())

# -------------------------------------------------------------------------
# 4) Byg Folium-kort
# -------------------------------------------------------------------------
m = folium.Map(location=[56, 10], zoom_start=7, tiles="cartodbpositron")

# Farv efter "pct_change_2010_2024"
folium.Choropleth(
    geo_data=merged,
    data=merged,
    ...
    fill_opacity=0.7,
    line_opacity=0.3,
    legend_name="Stigning (pct) 2010-2024"
).add_to(m)

# Nu laver vi let popup/tooltip for hver kommune
...
for _, row in merged.iterrows():
    geom = row["geometry"]
    if geom is None:
        continue  # Hvis ingen geometri pga. manglende match
    kommune_navn = row["Kommune"]
    pct = row["pct_change_2010_2024"]
    # Tekst til popup
    if pd.notna(pct):
        txt = f"<b>{kommune_navn}</b><br>Stigning: {pct:.1f}%"
    else:
        txt = f"<b>{kommune_navn}</b><br>Ingen data"
    ...
    gj.add_child(folium.Popup(txt, max_width=200))
    gj.add_to(m)

folium.LayerControl().add_to(m)

# -------------------------------------------------------------------------
# 5) Gem HTML-fil i ønsket mappe
# -------------------------------------------------------------------------
output_path = "C:/Users/ruk/OneDrive - Green Power Denmark/APP Stork/Kampagne/bolig/DST tal/Data til SoMe/danmarkskort.html"
m.save(output_path)
print("KORT GEMT:", output_path)
